...
impl Image {
    ...
    pub fn gamma_correction(&mut self) {
        for i in 0..self.height {
            for j in 0..self.width {
                let base_index = 3 * self.width * i + 3 * j;
                self.pixels[base_index] = self.pixels[base_index].powf(1.0 / 2.2);
    ...
    pub fn write_ppm(&self) {
        ...
        for i in 0..self.height {
            for j in 0..self.width {
                let base_index = 3 * self.width * i + 3 * j;
                let r = (255.0 * self.pixels[base_index]).clamp(0.0, 255.0) as i32;
